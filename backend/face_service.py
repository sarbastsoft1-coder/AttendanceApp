...
class FaceRecognitionService:
    ...
    def find_all_matches(
        ...
        NOTE: confidence = 1 - face_distance, and self.tolerance is a distance threshold.
        So a face is accepted when confidence >= (1 - self.tolerance).
        This keeps room-scan consistent with individual compare_faces (distance <= tolerance).
        """
        matches = []
        identified_user_ids = set()
        confidence_threshold = 1.0 - self.tolerance  # e.g. tolerance=0.6 → threshold=0.4

        for face_enc in unknown_encodings:
            user_id, name, confidence = self.find_best_match(known_encodings, face_enc)
            if user_id and user_id not in identified_user_ids and confidence >= confidence_threshold:
